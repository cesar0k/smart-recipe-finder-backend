...
def _with_owner(query: Select[tuple[Recipe]]) -> Select[tuple[Recipe]]:
    """Eager-load everything the Pydantic Recipe schema serializes."""
...
async def _set_recipe_ingredients(
    ...
    find-or-created. Duplicates in the same payload are skipped.
    """
    from app.models.recipe.recipe_ingredient import RecipeIngredient
    ...
    for idx, raw_name in enumerate(names):
        ...
        if ing is None or ing.id in seen:
            continue
        seen.add(ing.id)
        db.add(RecipeIngredient(recipe_id=recipe.id, ingredient_id=ing.id, position=idx))


async def _ensure_ingredients_loaded(db: AsyncSession, recipe: Recipe) -> None:
    """Eager-load recipe_ingredients + their .ingredient (avoids lazy="raise")."""
...
async def _ensure_images_loaded(db: AsyncSession, recipe: Recipe) -> None:
    """Eager-load Recipe.images after an insert/refresh."""
...
async def _add_recipe_images(
    ...
    """Append (full_url, thumb_url) pairs to recipe.images, continuing positions."""
...
async def _set_recipe_images(
    ...
    position is rewritten to the new order.
    """
    from app.models.recipe.recipe_image import RecipeImage
...
async def _ensure_cuisine_loaded(db: AsyncSession, recipe: Recipe) -> None:
    """Eager-load Recipe.cuisine_ref when the FK is set."""
...
def _apply_filters(
    ...
    """Apply ingredient / time / difficulty / cuisine filters to *query*.

    Ingredient filters go through the normalised recipe_ingredients ↔
    ingredients join. "include foo, bar" → recipe must match both;
...
async def get_distinct_cuisines(db: AsyncSession) -> list[str]:
    """Sorted names of cuisines actually used by at least one approved recipe."""
    from app.models.recipe.cuisine import Cuisine
...
async def create_recipe(
    ...
    *,
    ...
    # ingredients/cuisine/images travel through their own helpers below.
    recipe_data = recipe_in.model_dump(
        exclude={"ingredients", "cuisine", "image_urls", "thumbnail_urls"}
    )

    # Moderators/admins auto-approve; regular users go through the pending queue.

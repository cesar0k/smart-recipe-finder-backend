...
def _is_positive_only_intent(tag_filter: dict[str, Any]) -> bool:
    ...
    SQL-first works well when the tag uniquely identifies a large, well-covered
    category (vegan, vegetarian, meal_type). It works poorly for fields like
    spice_level where coverage in our dataset is sparse — better to let the
    vector find semantically similar docs and use tag as post-filter.
